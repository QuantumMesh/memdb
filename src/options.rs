...
impl Options {
    ...
    pub fn page_size(mut self, pagesize: u64) -> Self {
        if pagesize < 1024 {
            error!("Pagesize must be 1024 bytes minimum");
        }
    ...
    pub fn num_pages(mut self, numpages: usize) -> Self {
        if numpages < 4 {
            error!("num_pages must be at least 4");
        }
    ...
    pub fn open<P: AsRef<Path>>(self, path: P) -> Result<DB> {
        ...
        let db = Inner::open(file, self.page_size, self.flags)?;
        Ok(DB {
            inner: Arc::new(db),
        })
    }
...
mod tests {
    ...
    fn test_init_file() {
        ...
        let file = super::init_file(&path, 4096, 4, false).unwrap();
